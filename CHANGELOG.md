...
- Added independent Browser cards: opening Browser from HOME again creates another card with its own tabs, native viewport, focus/stacking identity, and persisted position. Cards move, resize, hide, and reopen independently, and the minimap, region movement, marquee selection, directional focus, and grouped dragging include every card. Agents get `browser_new_window`, `browser_activate_window`, and `browser_list_windows`, every browser tool accepts `browserId`, and a card claimed by one agent is refused to another. Existing single-card layouts migrate to the default card; all cards share the one Chromium website-login profile.
