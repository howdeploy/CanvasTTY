...
type Request = Pick<CreateSessionRequest, 'isolation' | 'cwd' | 'hostId' | 'allowSubagents' | 'role' | 'parentSessionId'>;
...
export class CapsuleLaunchService {
  ...
  async cleanup(id: string): Promise<void> { await this.storage.cleanup(id); }
}
