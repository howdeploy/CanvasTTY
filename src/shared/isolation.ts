...
export function assertIsolationRequest(value: unknown): asserts value is IsolationRequest | undefined {
  ...
  if (!["direct", "worktree", "container"].includes(item.mode as string) || Object.keys(item).some(key => key !== "mode" && !(item.mode === "worktree" && key === "ref") && !(item.mode === "container" && ['profileId', 'capsuleId'].includes(key)))) throw new Error("Invalid isolation request; execution paths are owned by the main process.");
  if (item.capsuleId !== undefined && (typeof item.capsuleId !== 'string' || !/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/u.test(item.capsuleId))) throw new Error('A saved capsule identity is required.');
