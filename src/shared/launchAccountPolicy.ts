...
import { accountConfiguredForRuntime, accountLaunchModel, accountRouteMaxDataClass, accountRunsOnHost, accountServiceCount, accountSupportsRuntime, assertAccountAliases } from "./providerAccountPolicy.ts";
import { accountPrivacyDecision, type PrivacyLaunchInput } from "./ambientPrivacy.ts";

/** Select using all eligibility constraints together, never a model-only first match. */
/** `privacy` describes a launch a person made: an account with only the provider's estimate then
 * follows the CLI-login rule (see accountPrivacyDecision). Without it every cap is enforced. */
export function selectLaunchAccount(accounts: readonly ProviderAccount[], provider: ProviderAccount["provider"], model: string | undefined, accountId: string | undefined, dataClass?: DataClass, binding?: { hostId?: string; forPlacement?: boolean; limit: 1 | 2 }, profiles: readonly ApiProfile[] = [], privacy?: PrivacyLaunchInput): ProviderAccount | undefined {
  const covers = (account: ProviderAccount, requested: DataClass): boolean => dataClassSatisfies(requested, accountRouteMaxDataClass(account, profiles, model))
    || (privacy !== undefined && accountPrivacyDecision(account, provider, requested, privacy, profiles, model).kind === "warn");
  ...
  const eligible = (account: ProviderAccount): ProviderAccount => {
    ...
    if (binding && !binding.forPlacement && !accountRunsOnHost(account, provider, requestedHost, profiles)) throw new Error(`Account ${account.id} is bound to host ${hostKey(account)}, not ${requestedHost}.`);
    ...
    if (!accountSupportsModel(account, accountLaunchModel(account, model, profiles))) {
      const alternatives = configured.filter((candidate) => {
        if (candidate.bindingRequired || (binding && !binding.forPlacement && !accountRunsOnHost(candidate, provider, requestedHost, profiles))) return false;
        try { return accountSupportsRuntime(candidate, provider, profiles) && accountSupportsModel(candidate, accountLaunchModel(candidate, model, profiles))
          && (dataClass === undefined || covers(candidate, dataClass)); } catch { return false; }
    ...
    if (dataClass !== undefined && !covers(account, dataClass)) throw new Error(`Account ${account.label} handles at most ${cap}; this task is ${dataClass}.`);
    if (binding) {
