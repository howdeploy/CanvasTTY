...
const actor = (id) => ({ kind: "agent", agentId: id, connectionId: `connection-${id}`, provider: "codex", terminalSessionId: `terminal-${id}`, cwd: "/tmp" });
...
async function fixture(t, directory) {
  ...
  const workspace = new BrowserWorkspace({
    ...
    createInstance(id, onState) {
      const state = { browserId: id, tabs: [], activeTabId: null, visible: false, agents: [], downloads: [], pendingDialog: null };
      let number = 0;
      ...
      const service = {
        commands: [], gate: null, viewport: null, focused: false,
        ...
        async close() { state.visible = false; onState(state); }, async dispose() {},
